...
def get_pdb_data(pdb_chains, data_dir="../data/pdb", subsample=20):
    ...
    CA_coords = np.array(CA_coords)[:subsample]
    res_label = np.array(res_label).reshape(len(res_label),1)[:subsample]
    

    return CA_coords, res_label

def get_Backbone_atom_coords(pdb_chains, data_dir="../data/pdb", seq_length=20):
    """
    ...
    atom_coords = np.array(atom_coords)[:seq_length]
    res_label = np.array(res_label).reshape(len(res_label), 1)[:seq_length]
...
def get_chi_angles(pdb_chains, data_dir="../data/pdb", subsample=20):
    ...
    chi_angles = np.array(chi_angles)[:subsample]
...
>>>>>>> 6c96ef524321c4bc8d3d861619e03349054c050c
